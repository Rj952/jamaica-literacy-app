    ...
    <div>
      ...
      <div className="bg-gradient-to-br from-tech-700 to-tech-600 text-white px-6 py-10">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-2 mb-3 text-tech-200">
            <Sparkles />
            <span className="text-sm font-medium">AI-Powered · NSC-Aligned · Jamaica Context</span>
          </div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-white mb-2">
            Lesson Builder
          </h1>
          <p className="text-tech-100 text-sm md:text-base max-w-xl">
            Generate complete, classroom-ready literacy lessons grounded in the NSC, NCLP, and Jamaica's
            bilingual classroom reality. Powered by Claude AI.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700">
                The Lesson Builder uses the Anthropic Claude API. Add your key in Settings to generate lessons.
              </p>
            </div>
            <button onClick={onOpenSettings} className="btn-gold text-xs px-3 py-1.5 flex-shrink-0">
              Add Key
            </button>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          >
            ...
            {/* Grade */}
            <div>
              <label htmlFor="grade-select" className="label">Grade Level *</label>
              <select
                id="grade-select"
                value={form.grade}
              ...
              >
                ...
                {['Early Primary (1–3)', 'Upper Primary (4–6)', 'Secondary (7–9)'].map(band => (
                  <optgroup key={band} label={band}>
                    {gradeLevels.filter(g => g.band === band.split(' ')[0] + ' ' + band.split(' ')[1]).map(g => (
            ...
            {/* Strand */}
            <div>
              <label htmlFor="strand-select" className="label">NSC Literacy Strand *</label>
              <select
                id="strand-select"
                value={form.strand}
                onChange={e => setField('strand', e.target.value)}
                className="select-field"
                required
              >
                <option value="">Select strand…</option>
                {literacyStrands.map(s => (
            ...
            {selectedStrand && (
              <div>
                <label htmlFor="at-select" className="label">Attainment Target</label>
                <select
                  id="at-select"
                  value={form.attainmentTarget}
                  onChange={e => setField('attainmentTarget', e.target.value)}
                  className="select-field"
                >
                  <option value="">AI selects appropriate target…</option>
                  {selectedStrand.attainmentTargets.map(t => (
            ...
            {/* Language Context */}
            <div>
              <label htmlFor="lang-select" className="label">Language Context</label>
              <select
                id="lang-select"
                value={form.langContext}
                onChange={e => setField('langContext', e.target.value)}
                className="select-field"
              >
                {languageContexts.map(l => (
                  <option key={l.value} value={l.value}>{l.label}</option>
                ))}
              </select>
              <p className="mt-1 text-xs text-forest-400">
                {languageContexts.find(l => l.value === form.langContext)?.description}
              </p>
            </div>

            {/* Duration */}
            <div>
              <label htmlFor="dur-select" className="label">Lesson Duration</label>
              <select
                id="dur-select"
                value={form.duration}
            ...
            {/* Topic */}
            <div>
              <label htmlFor="topic-input" className="label">Topic / Text / Theme</label>
              <input
                id="topic-input"
                type="text"
                value={form.topic}
                onChange={e => setField('topic', e.target.value)}
                placeholder="e.g. Anancy stories, weather, community helpers…"
            ...
            {/* Student Count */}
            <div>
              <label htmlFor="count-input" className="label">Number of Students</label>
              <input
                id="count-input"
                type="text"
                value={form.studentCount}
                onChange={e => setField('studentCount', e.target.value)}
                placeholder="e.g. 30 (Grades 1–3)"
                className="input-field"
              />
            </div>
            ...
            {/* Notes */}
            <div>
              <label htmlFor="notes-input" className="label">Special Considerations</label>
              <textarea
                id="notes-input"
                value={form.notes}
                onChange={e => setField('notes', e.target.value)}
                placeholder="e.g. Several students below grade level; limited devices; include BYOD activity…"
                rows={3}
            ...
            {!apiKey && (
              <p className="text-xs text-forest-400 text-center">
                API key required to generate lessons
              </p>
            )}
          </form>
          ...
          <div className="lg:col-span-3 flex flex-col gap-4">
            ...
            {lesson && (
              <div className="flex items-center justify-between">
                <span className="tag tag-forest">Lesson Generated ✓</span>
                <div className="flex gap-2">
                  ...
                  <button onClick={generate} className="btn-secondary text-xs px-3 py-1.5">
                    <Refresh /> Regenerate
                  </button>
                </div>
              </div>
            ...
            {/* Loading skeleton */}
            {loading && (
              <div className="card animate-pulse space-y-3" aria-live="polite" aria-label="Generating lesson…">
                <div className="h-5 bg-forest-100 rounded w-2/3"/>
                ...
                <div className="h-5 bg-forest-100 rounded w-1/2 mt-4"/>
                <div className="h-3 bg-forest-100 rounded w-full"/>
                <div className="h-3 bg-forest-100 rounded w-3/4"/>
                <div className="h-5 bg-forest-100 rounded w-1/3 mt-4"/>
                <div className="h-3 bg-forest-100 rounded w-full"/>
                <div className="h-3 bg-forest-100 rounded w-5/6"/>
                <div className="h-3 bg-forest-100 rounded w-4/5"/>
                <p className="text-center text-sm text-forest-400 pt-2">Crafting your Jamaica-context lesson…</p>
              </div>
            )}
            ...
            {lesson && !loading && (
              <article
                ...
                className="card lesson-prose overflow-y-auto max-h-[70dvh]"
                dangerouslySetInnerHTML={{ __html: renderMarkdown(lesson) }}
              />
            )}

            {/* Empty state */}
            {!lesson && !loading && !error && (
              <div className="card flex flex-col items-center justify-center text-center py-16 text-forest-400 border-2 border-dashed border-forest-100">
                <div className="text-5xl mb-4">📝</div>
                <p className="font-display text-lg text-forest-600 mb-1">Your lesson will appear here</p>
        ...
        {/* NSC Strand reference */}
        {selectedStrand && (
          <div className="card bg-forest-50 border border-forest-200">
            <h3 className="font-display text-sm font-semibold text-forest-800 mb-2">
              {selectedStrand.icon} NSC Reference: {selectedStrand.label} — Attainment Targets
            </h3>
            <ul className="grid sm:grid-cols-2 gap-1">
              {selectedStrand.attainmentTargets.map((t, i) => (
                <li key={i} className="text-xs text-forest-600 flex items-start gap-1.5">
                  <span className="text-forest-400 mt-0.5">›</span>{t}
                </li>
              ))}
            </ul>
          </div>
