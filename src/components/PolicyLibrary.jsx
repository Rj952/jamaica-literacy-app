  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <span className={`inline-flex items-center justify-center w-12 h-7 rounded-md text-white text-xs font-bold font-mono ${eraYearColor[policy.era]}`}>
          {policy.year}
        </span>
...
function PolicyModal({ policy, onClose }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="policy-modal-title" className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-forest-950/60 backdrop-blur-sm" onClick={onClose} aria-hidden="true" />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90dvh] overflow-y-auto z-10" tabIndex={-1}>
        <div className="px-6 py-5 border-b border-forest-100 sticky top-0 bg-white rounded-t-2xl z-10">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 id="policy-modal-title" className="font-display text-xl font-semibold text-forest-900 leading-snug">
                {policy.name}
              </h2>
              <p className="text-sm text-forest-500 mt-0.5">{policy.agency}</p>
            </div>
            <button onClick={onClose} aria-label="Close policy details" className="p-1.5 rounded-lg text-forest-400 hover:text-forest-700 hover:bg-forest-50 flex-shrink-0 transition-colors">
              <XIcon />
            </button>
          </div>
        </div>
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-gold-50 rounded-xl p-4">
              <h3 className="text-xs font-bold text-gold-700 uppercase tracking-wider mb-2">Curriculum Anchor</h3>
            ...
            <div className="bg-tech-50 rounded-xl p-4">
              <h3 className="text-xs font-bold text-tech-700 uppercase tracking-wider mb-2">Assessment Signal</h3>
          ...
          <div className="border border-forest-200 rounded-xl p-4">
            <h3 className="text-xs font-bold text-forest-600 uppercase tracking-wider mb-2">\u{1F3EB} Classroom Implication</h3>
            <p className="text-sm text-forest-700 leading-relaxed">{policy.classroomImplication}</p>
          </div>
          {policy.url && (
            <a href={policy.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-tech-600 hover:text-tech-800 hover:underline">
              <ExternalLink /> View primary source document
            </a>
          )}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div role="group" aria-label="Filter policies by era" className="flex flex-wrap gap-2">
          {eras.map(era => (
            <button key={era} onClick={() => setFilter(era)} aria-pressed={filter === era}
              className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
                filter === era ? 'bg-forest-800 text-white' : 'bg-white text-forest-600 border border-forest-200 hover:border-forest-400 hover:text-forest-800'
              }`}>
              {era === ALL_ERAS ? 'All Eras' : eraLabels[era]?.label}
              {era !== ALL_ERAS && <span className="ml-1.5 text-xs opacity-70">({policies.filter(p => p.era === era).length})</span>}
            </button>
          ))}
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" role="list"
          ...
          {filtered.map(policy => (
            <div key={policy.id} role="listitem"><PolicyCard policy={policy} onClick={setSelected} /></div>
          ))}
        </div>
