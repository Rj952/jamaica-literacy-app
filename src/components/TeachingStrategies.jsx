...
function StrategyCard({ strategy }) {
  ...
  return (
    <article className="card overflow-hidden p-0">
      ...
      <div
        className={`${headerBg[strategy.tagType]} text-white px-5 pt-5 pb-4`}
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-1.5 mb-2">
              <span className={`tag ${tagClasses[strategy.tagType]} bg-white/20 text-white border-0`}>
                {strategy.tag}
              </span>
              <span className="tag bg-white/10 text-white/80 border-0 text-xs">
                {strategy.gradeRange}
              </span>
            </div>
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex items-center justify-between text-xs text-forest-400 mb-1">
          <span>📌 {strategy.policyAnchor.split(';')[0]}</span>
        </div>

        <button
          onClick={() => setOpen(!open)}
          aria-expanded={open}
          aria-controls={`strategy-detail-${strategy.id}`}
        ...
        {open && (
          <div id={`strategy-detail-${strategy.id}`} className="mt-4 space-y-4">
            {/* Policy anchor */}
            <div className="text-xs text-forest-500 bg-forest-50 rounded-lg px-3 py-2">
              <strong>Policy Anchor:</strong> {strategy.policyAnchor}
            </div>
            <div className="text-xs text-forest-500 bg-gold-50 rounded-lg px-3 py-2">
              <strong>NSC Strand:</strong> {strategy.nscStrand}
            </div>

            {/* Tactics */}
            <div>
              <h4 className="font-display text-sm font-semibold text-forest-800 mb-3">
                Classroom Tactics
              </h4>
              <div className="space-y-3">
                {strategy.tactics.map((tactic, i) => (
                  <div key={i} className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-forest-100 text-forest-600 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                      {i + 1}
                    </span>
                    <div>
                      <p className="text-sm font-semibold text-forest-800">{tactic.name}</p>
                      <p className="text-sm text-forest-600 leading-relaxed mt-0.5">{tactic.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <h4 className="text-sm font-semibold text-amber-800 mb-2">💡 Teacher Tips</h4>
    ...
    <div>
      ...
      <div className="bg-white border-b border-forest-100 px-6 py-8">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-forest-500 text-sm md:text-base max-w-2xl">
            Evidence-grounded instructional strategy clusters for Jamaican literacy educators.
            Each strategy links to its national policy anchor, NSC strand, and provides
            immediately actionable classroom tactics.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 space-y-6">
        ...
        <div
          ...
          role="list"
          aria-label={`${filtered.length} teaching strategies`}
        >
        ...
        {/* Timetabled Reading callout */}
        <div className="bg-tech-50 border border-tech-200 rounded-xl p-5">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-tech-600 text-white flex items-center justify-center flex-shrink-0 text-lg">
              📅
            </div>
            <div>
              <h2 className="font-display text-base font-semibold text-tech-800 mb-1">
                2025 Timetabled Reading Initiative — What It Means for You
              </h2>
              <p className="text-sm text-tech-700 leading-relaxed">
                From 2025, reading is a <strong>timetabled subject</strong> for Grades 1–3 with a minimum of
                2 hours per week. Every Grade 1–3 teacher is expected to deliver explicit reading instruction
                covering <strong>fluency, vocabulary, word recognition, and reading for enjoyment</strong>.
                This is a shift from integrated literacy to protected instructional time. Use the
                Early Grade Structured Reading strategy as your framework.
              </p>
              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                {['Fluency', 'Vocabulary', 'Word Recognition', 'Reading for Enjoyment'].map(c => (
                  <span key={c} className="tag tag-tech">{c}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* JC/SJE guidance note */}
        <div className="bg-gold-50 border border-gold-200 rounded-xl p-5">
          <h2 className="font-display text-base font-semibold text-gold-800 mb-2">
            🇯🇲 Teaching in a Bilingual Classroom
          </h2>
          <p className="text-sm text-gold-700 leading-relaxed mb-2">
            Jamaica's classrooms are bilingual. The NSC explicitly requires students to
            <strong> recognise, value, and distinguish</strong> Jamaican Creole (JC) from
            Standard Jamaican English (SJE). This is not a deficit perspective — it is a
            <strong> linguistic asset approach</strong>.
          </p>
          <p className="text-sm text-gold-700 leading-relaxed">
            The Language Education Policy (2001, draft) endorses oral JC use in early years (K–3) to
            facilitate English literacy. Teachers should feel empowered to use JC orally to activate
            prior knowledge, then bridge deliberately into SJE academic register.
          </p>
        </div>
      </div>
    </div>
